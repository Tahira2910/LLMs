...
def get_custom_prompt():
    ...
    return ChatPromptTemplate.from_messages([
        SystemMessagePromptTemplate.from_template(
            "You are an Chatbot  designed to answer user's questions. Follow these guidelines:\n"
...
"8. Keep answers concise and focused."
        ...
        HumanMessagePromptTemplate.from_template(
            "Context:\n{context}\n\n"
            "Question: {question}\n\n"
            # "Provide a 9 to 10 lines precise and well-structured answer based on the context above. Ensure your response is easy to understand, includes examples where necessary, and is formatted in a way that students can use it for exams. If applicable, ask if the student needs further clarification."
            "Provide short, brief, complete, to the point, well-structured answer like a chatbot answering to a user question, based on the context above. Ensure your response is easy to understand."
        )
    ])
...
# def initialize_qa_chain():
#     if not st.session_state.qa_chain and st.session_state.vector_store:
#         llm = ChatOllama(model="deepseek-r1:1.5b", temperature=0.3, max_tokens=150)  # Limit response length
#         retriever = get_retriever()
#         st.session_state.qa_chain = RetrievalQA.from_chain_type(
#             llm,
#             retriever=retriever,
...
def chat_interface():
    st.title("Emerging Tech Grid")
    st.markdown("Chat with your textbook using Deepseek 1.5B")
